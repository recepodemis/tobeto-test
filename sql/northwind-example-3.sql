...
SELECT p.product_name, SUM(p.units_on_order + od.quantity) AS total_units
FROM products AS p
JOIN order_details AS od ON od.product_id = p.product_id
GROUP BY p.product_name
HAVING SUM(p.units_on_order + od.quantity) > 1000;
...
SELECT CONCAT(e.first_name, ' ',e.last_name),SUM(od.quantity)
FROM employees as e
...
SELECT *
FROM orders
WHERE shipped_date BETWEEN '1996-07-06' AND '1996-07-30';
